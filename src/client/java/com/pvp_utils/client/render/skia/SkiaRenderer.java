...
public class SkiaRenderer {
    ...
    public static void end(GuiGraphicsExtractor graphics, int guiWidth, int guiHeight) {
        if (USE_GL_BACKEND_FOR_FRAME) {
            try {
                GL_BACKEND.end();
            } finally {
    ...
    public static void endRegion(GuiGraphicsExtractor graphics) {
    ...
    public static void drawCached(GuiGraphicsExtractor graphics, int guiWidth, int guiHeight) {
    ...
    public static void drawCachedRegion(GuiGraphicsExtractor graphics) {
        pruneIdleSurfaces();
    ...
    private static boolean uploadSurface(Surface sourceSurface, DynamicTexture targetTexture, int uploadW, int uploadH) {
        ...
        try {
            ...
            RenderSystem.getDevice().createCommandEncoder()
                    .writeToTexture(gpuTexture, buf, 0, 0, 0, 0, uploadW, uploadH);
            return true;
        } finally {
            pixmap.close();
        }
